...
const generatedModules = (import.meta as ImportMeta & { glob: (pattern: string) => Record<string, () => Promise<unknown>> }).glob("../_generated/**/*.{ts,js}");
const localModules = (import.meta as ImportMeta & { glob: (pattern: string) => Record<string, () => Promise<unknown>> }).glob("./*.{ts,js}");
const modules = {
  ...generatedModules,
...
describe("demo sintética del catálogo", () => {
  it("prueba el flujo real y es idempotente al repetirlo", async () => {
    ...
    expect(primera).toEqual({
      ...
      consultas: { clases: 1, familias: 1, tipos: 1, unidades: 1, atributos: 2, opciones: 1, reglas: 0 },
    });
    ...
    }))).toEqual({ recursos: [{ identidad: primera.identificadorTecnico, activo: true, revision: 3 }] });
  });
});
