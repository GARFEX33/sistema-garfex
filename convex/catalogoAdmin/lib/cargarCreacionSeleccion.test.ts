...
async function seed(t: ReturnType<typeof convexTest>) {
  return t.run(async (ctx) => {
    ...
    const unidad = await ctx.db.insert("unidades", { clave: "METRO_LINEAL", nombre: "Metro Lineal", activo: true, revision: 1 });
...
describe("cargarCreacionSeleccion", () => {
  ...
  it("accepts active Metro Lineal without policy and rejects absent or inactive Units", async () => {
    const t = convexTest(schema, modules), fixture = await seed(t);
    ...
    expect(loaded).toMatchObject({ unitValid: true, unidad: { id: fixture.unidad, nombre: "Metro Lineal", activo: true } });
    ...
    expect(await t.run(ctx => cargarCreacionSeleccion(ctx, { ...input, unidadId: inactiveUnit }))).toMatchObject({ unitValid: false, unidad: { id: inactiveUnit, activo: false } });
    const absentGraph = await t.run(ctx => cargarCreacionSeleccion(ctx, { ...input, unidadId: "missing-unit" }));
    expect(absentGraph).toMatchObject({ unitValid: false });
    expect(absentGraph.unidad).toBeUndefined();
  });

  it("uses bounded indexed reads without policy or nonselected-Unit reads", () => {
    ...
    expect(source).toContain("ctx.db.get(input.unidadId");
    ...
    expect(source).not.toMatch(/catalogoRevisiones|catalogoTipoSnapshots|politicasUnidadRecurso|resolverUnidadesEfectivas|\.collect\(/);
  });
});
