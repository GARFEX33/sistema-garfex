...
import { adminConflict, adminDuplicateKey, adminInvalidReference, adminInvalidState } from "./lib/errors";
...
import { detectarConflictosReglas, detectarConflictosReglasSeleccion, type ReglaCondicional } from "../../src/catalogoRecursos/dominio/reglasCondicionales";
...
const detail = v.object({
  id: v.id("reglasAtributoRecurso"), tipoRecursoId: v.id("tiposRecurso"), atributoCondicionId: v.id("atributosRecurso"), opcionCondicionId: v.optional(v.id("opcionesAtributo")), valorPermitidoCondicionId: v.optional(v.id("valoresPermitidosAtributo")), atributoAfectadoId: v.id("atributosRecurso"), aplicabilidad: applicability,
...
async function scope(ctx: Db, typeId: Id<"tiposRecurso">) {
  ...
  return { type: type!, family: family!, selected: new Map([...byDefinition.values()].map(row => [String(row._id), row] as const)) };
}

async function mappedValue(ctx: MutationCtx, optionId: Id<"opcionesAtributo">) {
  const values = await ctx.db.query("valoresPermitidosAtributo").withIndex("porOpcionDeValor", q => q.eq("opcionAtributoIdIndex", optionId)).take(2);
  return values.length === 1 && values[0].valor.kind === "OPCION" && values[0].valor.opcionAtributoId === optionId ? values[0] : null;
}

async function resolveConditionValue(ctx: MutationCtx, rule: Pick<Rule, "tipoRecursoId" | "atributoCondicionId" | "opcionCondicionId" | "valorPermitidoCondicionId" | "atributoAfectadoId">, requireActive = false) {
  ...
  let option: Doc<"opcionesAtributo"> | null = null;
  if (rule.opcionCondicionId !== undefined) {
    option = await ctx.db.get(rule.opcionCondicionId);
    if (!definition || definition.tipoDato !== "OPCION" || !option || option.definicionAtributoId !== definition._id || (requireActive && !option.activo))
      adminInvalidReference({ entityKind: "reglasAtributoRecurso", field: "opcionCondicionId", reference: { kind: "opcionesAtributo", id: rule.opcionCondicionId }, reason: "option must be active and owned by an OPCION condition definition" });
  }
  let value = rule.valorPermitidoCondicionId === undefined ? undefined : await ctx.db.get(rule.valorPermitidoCondicionId);
  if (rule.valorPermitidoCondicionId !== undefined && (!value || !definition || value.definicionAtributoId !== definition._id || ((requireActive || option !== null) && !value.activo)))
    adminInvalidReference({ entityKind: "reglasAtributoRecurso", field: "valorPermitidoCondicionId", reference: { kind: "valoresPermitidosAtributo", id: rule.valorPermitidoCondicionId }, reason: "allowed value must be active and owned by the condition definition" });
  if (option) {
    const mapped = await mappedValue(ctx, option._id);
    if (!mapped || (value && (value._id !== mapped._id || value.valor.kind !== "OPCION" || value.valor.opcionAtributoId !== option._id)))
      adminInvalidReference({ entityKind: "reglasAtributoRecurso", field: "valorPermitidoCondicionId", reason: "legacy option must map to the supplied allowed value" });
    value = mapped;
  }
  return value?._id;
}
...
async function validateSet(ctx: MutationCtx, typeId: Id<"tiposRecurso">, candidate?: Rule) {
  ...
  const conflict = [...detectarConflictosReglas(all.map(domain)), ...detectarConflictosReglasSeleccion(all.map(domain))].find(Boolean);
...
async function item(ctx: Db, row: Rule) {
  ...
  const clazz = await ctx.db.get(owner.family.claseRecursoId);
  if (!row.activo) reasons.push("INACTIVE");
  else if (!owner.type.activo || !owner.family.activo || !clazz?.activo) reasons.push("HIERARCHY_INACTIVE");
  else if (!condition || !affected || !condition.activo || !affected.activo) reasons.push("ASSIGNMENT_INACTIVE");
  ...
  else if (row.valorPermitidoCondicionId !== undefined && !(await ctx.db.get(row.valorPermitidoCondicionId))?.activo) reasons.push("ALLOWED_VALUE_INACTIVE");
  else effective = true;
  return { id: row._id, tipoRecursoId: row.tipoRecursoId, atributoCondicionId: row.atributoCondicionId, opcionCondicionId: row.opcionCondicionId, valorPermitidoCondicionId: row.valorPermitidoCondicionId, atributoAfectadoId: row.atributoAfectadoId, aplicabilidad: row.aplicabilidad, activo: row.activo, revision: row.revision, effective, effectiveReasons: reasons };
}

async function validateActive(ctx: MutationCtx, row: Rule) { await resolveConditionValue(ctx, row, true); await validateSet(ctx, row.tipoRecursoId, row); }

export const crearReglaAtributo = mutation({
  args: { tipoRecursoId: v.id("tiposRecurso"), atributoCondicionId: v.id("atributosRecurso"), opcionCondicionId: v.optional(v.id("opcionesAtributo")), valorPermitidoCondicionId: v.optional(v.id("valoresPermitidosAtributo")), atributoAfectadoId: v.id("atributosRecurso"), aplicabilidad: applicability, activo: v.optional(v.boolean()) },
  ...
  handler: async (ctx, args) => {
    ...
    const valorPermitidoCondicionId = await resolveConditionValue(ctx, args);
    const duplicate = await ctx.db.query("reglasAtributoRecurso").withIndex("porTipoYCondicionYOpcionYValorYAfectadoYAdminSort", q => q.eq("tipoRecursoId", args.tipoRecursoId).eq("atributoCondicionId", args.atributoCondicionId).eq("opcionCondicionId", args.opcionCondicionId).eq("valorPermitidoCondicionId", valorPermitidoCondicionId).eq("atributoAfectadoId", args.atributoAfectadoId)).first();
    if (duplicate) adminDuplicateKey({ entityKind: "reglasAtributoRecurso", normalizedIdentity: `${args.tipoRecursoId}|${args.atributoCondicionId}|${args.opcionCondicionId ?? ""}|${valorPermitidoCondicionId ?? ""}|${args.atributoAfectadoId}` });
    const id = await ctx.db.insert("reglasAtributoRecurso", { ...args, valorPermitidoCondicionId, activo: args.activo ?? false, revision: 1 });
...
export const actualizarReglaAtributo = mutation({
  args: { reglaAtributoRecursoId: v.id("reglasAtributoRecurso"), expectedRevision: v.number(), tipoRecursoId: v.optional(v.id("tiposRecurso")), atributoCondicionId: v.optional(v.id("atributosRecurso")), opcionCondicionId: v.optional(v.union(v.id("opcionesAtributo"), v.null())), valorPermitidoCondicionId: v.optional(v.union(v.id("valoresPermitidosAtributo"), v.null())), atributoAfectadoId: v.optional(v.id("atributosRecurso")), aplicabilidad: v.optional(applicability) },
  returns: changeResultValidator(detail),
  handler: async (ctx, args) => { const result = await applyRevisionedUpdate<Rule, typeof args, { aplicabilidad?: Rule["aplicabilidad"] }>({ load: () => ctx.db.get(args.reglaAtributoRecursoId), expectedRevision: args.expectedRevision, entity: entity(args.reglaAtributoRecursoId), immutable: { tipoRecursoId: args.tipoRecursoId, atributoCondicionId: args.atributoCondicionId, opcionCondicionId: args.opcionCondicionId === null ? undefined : args.opcionCondicionId, valorPermitidoCondicionId: args.valorPermitidoCondicionId === null ? undefined : args.valorPermitidoCondicionId, atributoAfectadoId: args.atributoAfectadoId }, changes: args, normalize: changes => changes.aplicabilidad === undefined ? {} : { aplicabilidad: changes.aplicabilidad }, current: row => args.aplicabilidad === undefined ? {} : { aplicabilidad: row.aplicabilidad }, validate: async next => { if (next.aplicabilidad === "CONDITIONAL") adminInvalidState({ entity: entity(next._id), field: "aplicabilidad", reason: "conditional rules cannot produce CONDITIONAL" }); if (next.activo) await validateActive(ctx, next); }, patch: next => ctx.db.patch(next._id, { aplicabilidad: next.aplicabilidad, revision: next.revision }) }); return { disposition: result.disposition, item: await item(ctx, result.item) }; },
});
